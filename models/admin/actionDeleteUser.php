...
if ($id>0) {
    ...
    $query = $pdo->prepare('SELECT title FROM articles WHERE id_users=:id');
    $query->execute(["id"=>$id]); 
    $listArticles = $query->fetchall(PDO::FETCH_ASSOC);

    $query = $pdo->prepare('SELECT reply FROM comments WHERE id_users=:id');
    $query->execute(["id"=>$id]); 
    $listComments = $query->fetchall(PDO::FETCH_ASSOC);

    if (count($listArticles)>0) {
        $isSuccess=false;
        $arr = array("msg"=>"invalid action, this user is already used. Delete articles using this user","error"=>true);
    ...
    if (count($listComments)>0) {
        $isSuccess=false;
        $arr = array("msg"=>"invalid action, this user is already used. Delete comments using this user","error"=>true);
        echo(json_encode($arr));
        return;
    }
